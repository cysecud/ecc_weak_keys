...
pub type MathResult = Option<U224>;
type RootsResult = [Option<U224>;2];
impl <'a>Scalar<'a>for U224{
    const ORDER:&'a str = "224";
    const SIZE:usize = 7;
}
...
impl U224 {
    ...
    /*sistemare */
    /*     /*Algorithm 10.25 Fast reduction for special form moduli [Handbook Cohen] */
        let k=U256::from_dec_str(Self::ORDER).expect("error");
        let c=Self::from(2).pow(k)-rhs;
        let mut q0=self/(U256::from(2).pow(k)); 
        let mut r0=self-q0*U256::from(2).pow(k);
        let mut r=r0;
        let mut i=Self::zero();
        while q0>Self::zero() {
            println!("i is {}",i);
            ...
            println!("r0 is {}",r0);
            println!("r is {}",r);
            let q1=q0*c/(Self::from(2).pow(k));
            let r1=q0*c-q1*(Self::from(2).pow(k));
            /* println!("q1 is {}",q1);
            println!("r1 is {}",r1); */
            if i%2==U256::one() {if r<r1 {r=r+rhs-r1} else {r=r-r1}}
            else {if r+r1<rhs {r=r+r1} else {r=r+r1-rhs}}
            q0=q1;        
           i+=Self::one();
        }  
        while r>=rhs {r=r-rhs}
