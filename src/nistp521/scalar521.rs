...
impl <'a>Scalar<'a>for U521{
    const ORDER:&'a str = "521";
    const SIZE:usize = 18;
}
...
impl U521 {
    ...
    pub fn mod_fast(mut self, rhs: Self) -> Self{
        if self<rhs {self} 
        else if self==rhs {Self::zero()} 
        else {
        loop{
            let divmod=self.div_mod(rhs+1);
            self=divmod.0+divmod.1;
        if self<rhs {break self;} else if self==rhs {break Self::zero()
        }
        }
        }}
pub fn random()->Self {
