...
pub struct FieldP256k1{
    pub num:U256,
}
impl Add for FieldP256k1{
    type Output = Self;
    fn add(self,rhs:Self) -> Self::Output{
        let sum=self.num.mod_fast(Self::prime())+rhs.num.mod_fast(Self::prime());
        FieldP256k1{num:sum.mod_fast(Self::prime())}}
}  
impl Mul for FieldP256k1{
    type Output = Self;
    fn mul(self, rhs: Self) -> Self::Output {
        let mul=self.num.mod_fast(Self::prime())*rhs.num.mod_fast(Self::prime());
        FieldP256k1{num:mul.mod_fast(Self::prime())}}
}
impl Neg for FieldP256k1{
    type Output = Self;
    fn neg(self) -> Self::Output {
        FieldP256k1 {num:Self::prime()-self.num}
    }
}
impl Sub for FieldP256k1{
    type Output = Self;
    fn sub(self, other: Self) -> Self {
            FieldP256k1 {num: (self.num + other.neg().num).mod_fast(Self::prime())}
    }
}
impl <'a>FieldElement<'a,U256> for FieldP256k1{
    ...
    fn double(&mut self)->FieldP256k1 {
    ...
    fn power(&mut self,n: U256) -> Self {
        ...
        let mut p0=FieldP256k1::new(U256::one());
        let mut p1=*self;
        for i in bin.iter() {
            if *i==U256::zero() {
                p1=p1*p0;
    ...
    fn inverse (&mut self)-> FieldP256k1 {
        FieldP256k1::power(self, Self::prime()-U256::from(2))
}
fn rand_mod()-> FieldP256k1{
    let prime=U256::from_dec_str(Self::PRIME).expect("error");
