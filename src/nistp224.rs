...
impl Curve<FieldP224,U224,MathResult> for P224 {
    ...
    fn ellnegate_proj(&self,p:ProjectivePoint<FieldP224>)->ProjectivePoint<FieldP224> {
     ...
     fn zn_prim_root_gen_order(&self)-> U224{
            U224::from_dec_str(ImplicitP224::PRIME_ROOT).expect("error in primitive root of generator's order!")
    }
     fn gen_order(&self)-> U224{
        U224::from_dec_str(ImplicitP224::PRIME).expect("error in generator's order P!")
    }
 ...
 */     let a=FieldP224::new(U224::from_dec_str(Self::A).expect("error in A"));
    ...
    fn key_pairs(&self)->KeyPairs<U224,AffinePoint<FieldP224>> {
...
fn bsgs(&self,q:&mut AffinePoint<FieldP224>, d:U224)->MathResult {
	...
	'outer: for i in 0..m.as_u128() {
        bs.push((i,self.ellmul(&mut self.jc_to_affine(self.generator()), ImplicitP224::power(&mut zd,U224::from(i)).num)));
